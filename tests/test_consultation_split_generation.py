...
def test_prepares_one_mixed_mode_request_with_shared_sources_once_and_deterministic_estimate():
    ...
    assert first.output_token_cap == 2 * NOTE_GENERATION_LENGTH_TOKEN_CAPS["normal"]
...
def test_prepares_six_topics_and_counts_shared_input_once():
    ...
    assert prepared.output_token_cap == 6 * NOTE_GENERATION_LENGTH_TOKEN_CAPS["normal"]
...
@pytest.mark.parametrize("payload", [
    ...
    lambda topics: {"notes": [{"topic_uuid": str(topics[0].topic_uuid), "mode": "freeform", "content": "x", "title": "no"}, {"topic_uuid": str(topics[1].topic_uuid), "mode": "structured", "content": {"problem": "p", "tasks": "t"}}]},
    lambda topics: {"notes": [{"topic_uuid": str(topics[0].topic_uuid), "mode": "freeform", "content": "x"}, {"topic_uuid": str(topics[0].topic_uuid), "mode": "structured", "content": {"problem": "p", "tasks": "t"}}]},
    lambda topics: {"notes": [{"topic_uuid": str(uuid4()), "mode": "freeform", "content": "x"}, {"topic_uuid": str(topics[1].topic_uuid), "mode": "structured", "content": {"problem": "p", "tasks": "t"}}]},
    lambda topics: {"notes": [{"topic_uuid": str(topics[0].topic_uuid), "mode": "structured", "content": {"problem": "p", "tasks": "t"}}, {"topic_uuid": str(topics[1].topic_uuid), "mode": "structured", "content": {"problem": "p", "tasks": "t"}}]},
    lambda topics: {"notes": [{"topic_uuid": str(topics[0].topic_uuid), "mode": "freeform", "content": "[PHI-1]"}, {"topic_uuid": str(topics[1].topic_uuid), "mode": "structured", "content": {"problem": "p", "tasks": "[PHI-2]"}}]},
    lambda topics: {"notes": [{"topic_uuid": str(topics[0].topic_uuid), "mode": "freeform", "content": "x"}, {"topic_uuid": str(topics[1].topic_uuid), "mode": "structured", "content": {"problem": "p"}}]},
    lambda topics: {"notes": [{"topic_uuid": str(topics[0].topic_uuid), "mode": "freeform", "content": "x"}, {"topic_uuid": str(topics[1].topic_uuid), "mode": "structured", "content": {"problem": "p", "tasks": 3}}]},
    lambda topics: {"notes": [{"topic_uuid": str(topics[0].topic_uuid), "mode": "freeform", "content": "x"}, {"topic_uuid": str(topics[1].topic_uuid), "mode": "structured", "content": {"problem": "p", "tasks": "t", "other": "x"}}]},
])
...
def test_parser_rejects_duplicate_json_keys_malformed_json_and_per_note_bound_without_content_leaks():
    ...
    cases = [
        '{"notes":[{"topic_uuid":"%s","topic_uuid":"%s","mode":"freeform","content":"x"}]}' % (topic.topic_uuid, topic.topic_uuid),
        '{"notes":[{"topic_uuid":"%s","mode":"structured","content":{"problem":"a","problem":"b"}}]}' % topic.topic_uuid,
        '{bad json',
        json.dumps({"notes": [{"topic_uuid": str(topic.topic_uuid), "mode": "freeform", "content": "secret-too-large"}]}),
    ]
    for raw in cases:
        with pytest.raises(AppError) as exc:
...
def test_output_bound_tracks_frozen_note_option():
    ...
    prepared = prepare_split_generation_request(**snapshots)
    assert prepared.output_token_cap == 1600
...
def test_parser_applies_total_response_cap_to_mapping_inputs_too():
    ...
    payload = {"notes": [{"topic_uuid": str(topic.topic_uuid), "mode": "freeform", "content": "x" * CONSULTATION_SPLIT_GENERATION_MAX_RESPONSE_CHARS}]}
    with pytest.raises(AppError) as exc:
...
def test_partial_parser_salvages_only_identified_valid_topics():
    ...
    parsed = parse_split_generation_partial({"notes": [
...
def test_structured_parser_allows_empty_optional_sections():
    ...
    parsed = parse_split_generation({"notes": [{
...
def test_partial_parser_rejects_ambiguous_outer_envelope():
    topic = SplitGenerationTopic(uuid4(), "freeform")
    with pytest.raises(AppError) as exc:
        parse_split_generation_partial({"notes": [
