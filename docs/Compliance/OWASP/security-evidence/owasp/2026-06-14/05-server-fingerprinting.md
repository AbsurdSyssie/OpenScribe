...
| Public API schema | `/docs`, `/redoc`, and `/openapi.json` now default to full system-admin auth in production while staying public in local/test. Production unauthenticated retest returned `401` on all three docs/schema routes. | Resolved | `09-findings-and-remediation.md#owasp-2026-06-14-009` |
...
All other gaps from the initial 2026-06-14 capture are now closed or accepted: HSTS is single-owned by Cloudflare, COEP and Permissions-Policy are deployed and ZAP-confirmed, public metadata files are served with correct content types and no cookies, CSRF hidden tokens are rendered on public forms and ZAP-confirmed, and public `/docs`/`/openapi.json` exposure is remediated with production unauthenticated retest evidence.
