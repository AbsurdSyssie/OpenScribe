...
- Compatibility URL `/admin?tab=providers` shows an explicit team picker instead of silently returning Admin home. Selecting a team maps to that team's `provider-policy` tab; invalid or missing team IDs stay on the picker without guessing scope.
