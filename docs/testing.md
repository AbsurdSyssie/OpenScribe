...
- template bundle coverage for UTF-8/format/version/size rejection, warn-and-ignore additive fields, strict unknown structured fields, freeform configuration rejection, exact-copy detection, deterministic suffixes, selected-subset validation and rollback, metadata-only audit, scoped export, and leader-only Team import
- template bundle workspace coverage for mixed Personal/Team export selection, Personal-default destination choice, uploaded and pasted JSON sharing preflight/commit, single outer-fence removal for pasted input, clean single-template immediate import, warning/conflict/multi-template review, original-payload resubmission, AI-help clipboard/manual-copy behavior and confidential-data warning, CSRF-protected requests, text-only rendering of imported values, and bottom-anchored accessible rail controls
- Quick Action bundle coverage for current-Team/Personal visibility, leader-only Team import, latest-freeform-version export, duplicate planning, atomic commit and rollback, metadata-only audit, authenticated routes, upload/paste review, and workspace export selection
- Smart Phrase bundle coverage for Personal-only authority, strict trigger/expansion validation, deterministic length-safe `_COPY_N` collisions, usage-metadata reset, atomic commit and rollback, metadata-only audit, authenticated routes, upload/paste review, export selection, and search restoration
