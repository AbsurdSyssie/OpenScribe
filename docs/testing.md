...
- Settings workspace tests verify `/settings` requires full authentication, redirects system admins, uses `no-store`, groups Smart phrases under My Library, orders Account before Preferences, keeps note length/detail visible while model controls remain in closed Advanced disclosure, preserves saved model selection when writing-style preferences change, limits normal users to non-management sections, shows same-team templates read-only with copy-to-personal but without team management controls, excludes cross-team templates, rejects crafted normal-user team-template edits, renders owner name/email/password forms, requires password and active TOTP reauthentication for sensitive changes, enforces normalized email uniqueness, rotates sessions after email/password changes, excludes submitted account values from audit details, rejects missing CSRF, exposes leader-only sections through existing service gates, omits leader hard-delete, and preserves validated `return_view=settings` redirects and template-editor return paths.
