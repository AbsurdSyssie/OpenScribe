...
- The consultation-split persistence boundary stores proposals, draft and confirmed topic titles, confirmed plans, clinical/source/template/PII/provider snapshots, requests, responses, and recoverable output as owner-encrypted text. It allocates the row UUID before encryption and binds ciphertext to owner, table, field, and row UUID. Its owner/team/transcript fields are not an access grant: active owner scope, denormalized ancestors, and copied retention deadline are checked before decrypting. System administrators cannot create or read split content.
- Split attempts, usage, and outbox rows store only execution IDs, phase kinds, quota estimates, provider adapter/model metadata, timestamps, and safe error codes. They never store content or credentials. Confirmation atomically creates the bundled-generation execution, reservation, and outbox row with encrypted frozen request/provider snapshots. The runtime resolves credentials outside the final database transaction and commits submission before one provider call. A trustworthy durable mixed response retains only independently validated encrypted outcomes. It may queue one automatic recovery using the initial frozen provider snapshot; submitted work with no durable response never auto-retries. Verification waits for a terminal automatic-recovery decision and never snapshots survivors while recovery is active. A clinician retry uses the current eligible provider and a fresh execution, reservation, and outbox row. Keep Available is database-only, idempotent, and blocked while a recovery or verification is active. Disabled manual Retry and Keep requests fail before root lookup and create no work. Workspace/API projections contain only safe counts, flags, IDs, and lifecycle state; they expose no outcome, provider, quota, or recovery content. All materialized notes remain clinician-review drafts.
...
- Consultation-split verification stores no verifier reasoning, patch text, provider response, or source content in batch metadata, APIs, SSE, or logs. Provider input, recoverable response, accepted output, and any corrected output use the owning user's encrypted fields.
...
The maintained application cookie and Web Storage inventory is in [cookies-browser-storage.md](cookies-browser-storage.md). It separates application behavior from deployment-specific Cloudflare behavior and records the current minimisation finding for persistent transcript-linked browser metadata.
